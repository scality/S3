...
class KMS {
       ...
       if (algorithm === 'AES256' || algorithm === 'aws:kms') {
           this.createBucketKey(bucketName, log, (err, masterKeyId) => {
                ...
                if (algorithm === 'aws:kms') {
                    ...
                    // if no key id is specified when using aws:kms one was generated for you
                    // as such we reuse the generated bucket key for the kms key
                    if (!configuredMasterKeyId) {
                        serverSideEncryptionInfo.configuredMasterKeyId = serverSideEncryptionInfo.masterKeyId;
                    } else {
    ...
    //         if (!configuredMasterKeyId) {
    //             // TODO Replace with same error as bad putbucketencryption
